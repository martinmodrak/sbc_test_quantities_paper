---
title: "Simple Bernoulli Examples - Continuous"
...
res_phiB <- compute_SBC(ds, backend_phiB, keep_fits = FALSE,
                        gen_quants = gq_simple, globals = my_globals,
                      cache_mode = "results",
...
```{r}
invcdf_test_lincomb_0 <- function(u) {
  1.5 - 0.5 * sqrt(9 - 8 * u)
}

invcdf_test_lincomb_1 <- function(u) {
  -0.5 + 0.5 * sqrt(1 + 8 * u)
}

backend_test_lincomb <- my_backend_func_invcdf(invcdf_test_lincomb_0, invcdf_test_lincomb_1)


res_test_lincomb <- compute_SBC(ds_large, backend_test_lincomb, keep_fits = FALSE,
                        gen_quants = gq_simple, 
                        globals = my_globals,
                      cache_mode = "results",
                      cache_location = file.path(cache_dir, "lincomb"))

plot_rank_hist(res_test_lincomb)
plot_ecdf_diff(res_test_lincomb)
