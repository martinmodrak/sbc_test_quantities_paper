...
# If true, additional test quantities using data simulated from the
# samples and their distance to ture data are used.
include_sims <- FALSE

# Setup caching of SBC results for faster iterations
if(include_sampling_scores) {
  cache_dir <- "./_samp_scores_SBC_cache"
} else if(include_sims) {
  cache_dir <- "./_SBC_cache_sims"
} else {
...
if(include_sims) {
  sim_distance_f <- function(y, mu, sigma, K, fun) {
      y_times <- matrix(rep(t(y),K),ncol=ncol(y),byrow=TRUE)
      N <- nrow(y_times)
      yrep <- mvtnorm::rmvnorm(N, mean = mu, sigma = sigma)
      stopifnot(identical(dim(y_times), dim(yrep)))
      mean(fun(yrep - y_times))
  }
  
  square <- function(x) {x^2}
  
  quants_sims <- derived_quantities(`sim abs` = sim_distance_f(y, mu, mvn_sigma, 1, abs),
                                    `sim sq` = sim_distance_f(y, mu, mvn_sigma, 1, square),
                                    `sim abs 10` = sim_distance_f(y, mu, mvn_sigma, 10, abs),
                                    `sim sq 10` = sim_distance_f(y, mu, mvn_sigma, 10, square),
                                    .globals = c("sim_distance_f", "square"))

  quants <- bind_derived_quantities(quants, quants_sims)
...
if(include_sims) {
  quants_non_mon <- bind_derived_quantities(quants_non_mon, quants_sims)
}
