...
$$
\pi_\text{obs}(y | \theta) \pi_\text{prior}(\theta) = \pi_\text{joint}(y, \theta) = \pi_\text{post}(\theta | y) \pi_\text{marg}(y)
$$

$$
\pi_\text{obs}(y | \tilde\theta) \pi_\text{prior}(\tilde\theta) = \pi_\text{marg}\left(y \right)\pi_\text{post}(\tilde\theta | y)
$$
...
$$
h_1 = \frac{5 h_0 - 4}{8 h_0 - 7}
$$


$$
2x = - \left(\Phi^{-1}(x | 0) - 1\right)^2 + 2 + \frac{3(x - 1)}{4(1 - h_1)} \\
\left(\Phi^{-1}(x | 0) - 1\right)^2 = - 2x + 2 + \frac{3(x - 1)}{4(1 - h_1)} \\
1 - \Phi^{-1}(x | 0) = \sqrt{ 2 - 2x + \frac{3(x - 1)}{4(1 - h_1)}} \\
\Phi^{-1}(x | 0)  = 1 - \sqrt{ 2 - 2x + \frac{3(x - 1)}{4(1 - h_1)}} \\
...
h_1 \leq x < h_0 & \implies \Phi^{-1}(x | 0) = 1 - \frac{1}{2}\sqrt{\frac{x - 1}{h_0 - 1}} \\
h_0 \leq x < h_1 & \implies \Phi^{-1}(x | 1) = \frac{1}{2}\sqrt{\frac{3 - 4h_0 + x(8 h_0 - 7)}{h_0 - 1}}
\end{align*}
$$
...
$$
a = \phi\left(\frac{1}{3}|0\right), b = \phi\left(\frac{1}{3}|1\right)
$$

$$
q_{\phi,f_1}(x | 0) = \begin{cases}
   \frac{2x}{3 a} & x \leq a \\
   \frac{2}{3} + \frac{x - a}{3\left(1 - a\right)} & \text{otherwise}
\end{cases} \\
q_{\phi,f_1}(x | 1) = \begin{cases}
   \frac{x}{3 b} & x \leq b \\
   \frac{1}{3} + \frac{2\left(x - b\right)}{3\left(1 - b\right)} & \text{otherwise}
\end{cases}
$$

The SBC condition then becomes:

$$
2x = \begin{cases}
\frac{2x}{3 a} + \frac{x}{3 b} & x \leq \min\left\{a,b\right\} \\
\frac{2x}{3 a} + \frac{1}{3} + \frac{2\left(x - b\right)}{3\left(1 - b\right)} & b < x \leq a
\end{cases}
$$
For the case $ x \leq \min\left\{a,b\right\}$ the SBC condition becomes:

$$
2x = \frac{2x}{3a} + \frac{x}{3b}
$$

this can hold for all $x$ in the range only if

$$
2 = \frac{2}{3a} + \frac{1}{3b} \\
a b - \frac{1}{3}b - \frac{1}{6}a = 0
$$

Now we can inspect the case $ x > \max\left\{a,b\right\}$ where the SBC condition becomes.

$$
2x = 1 + \frac{x-a}{1-a} +\frac{2\left(x - b\right)}{3\left(1 - b\right)} 
$$

Focusing on the coefficient for $x$ in the above equation yields

$$
1 - \frac{1}{6(1-a)} - \frac{1}{3(1-b)} = 0 \\
6ab - 4a - 5b + 3
$$
This is a set of two equations which has only two solutions:
