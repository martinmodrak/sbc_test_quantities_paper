...
$$
\forall \tilde x \in [0,1], y \in Y, i \in \{0, \dots, M\}: \\
\mathbb{P}\left(f(\theta_i, y) <  C_f^{-1}(\tilde x | y)\right) =
\int_0^1 \mathrm{d} x\:
  \mathbb{I} \left[C^{-1}_{\phi,f}(x | y) < C_f^{-1}(\tilde x | y)\right] = \\
  = \int_0^1\mathrm{d}x\:
  \mathbb{I} \left[x < C_{\phi,f}\left( C_f^{-1}(\tilde x | y) | y \right) \right] = \int_0^{q^{-1}_{\phi,f}(\tilde x | y)}\mathrm{d}x \: 1 =
  q^{-1}_{\phi,f}(\tilde x | y)
$$

$$
\mathbb{P}\left(f(\theta_i, y) < f(\tilde\theta, y)\right) = C_{\phi,f}(f(\tilde\theta, y))
$$
