...
```{r, fig.width=hist_plot_width + 1, fig.height=hist_plot_height / 2}
...
```{r}
res_corr_quants  <- compute_SBC(ds, backend_uncorr, keep_fits = FALSE,
                        globals = my_backend_mvn_globals,
                        gen_quants = quants,
                        cache_mode = "results", 
                        cache_location = file.path(cache_dir, "corr_quants"))
```
...
```{r results_corr_quants}
plot_rank_hist(res_corr_quants)
plot_ecdf_diff(res_corr_quants)
```
