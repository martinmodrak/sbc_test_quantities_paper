...
sampling_func_non_mon <- function(y, N_samples) {
  res <- sampling_func_correct(y, N_samples)
  K <- ncol(y)
  ...
  for(k in 1:K) {
    ...
    if(mean(y[,k]) > 0) {
      mean_diff <- mean(diff(sort(res_k)))
      res_k <- res_k + 2 * mean_diff
    }

    
    # qs_1 <- quantile(res_k, probs = c(0.1, 0.3))
    # qs_2 <- quantile(res_k, probs = c(0.7, 0.9))
    # ids_1 <- res_k > qs_1[1] & res_k < qs_1[2]
    # ids_2 <- res_k > qs_2[1] & res_k < qs_2[2]
    # if(y[,k] > 0) {
    #   res_k[ids_1] <- res_k[ids_1] + abs(rnorm(sum(ids_1), sd = 0.2))
    # } else {
    #   #res_k[ids_2] <- res_k[ids_1]
    # }
    
    # qs <- quantile(res_k, probs = c(0.2, 0.4))
    # ids_to_change <- res_k > qs[1] & res_k < qs[2]
    # if(mean(y[,k]) > 0) {
    #   samp_prob <- rank(res_k[ids_to_change])
    #   res_k[ids_to_change] <- sample(res_k[ids_to_change], size = sum(ids_to_change), replace = TRUE, prob = samp_prob)
    # } else {
    #   #samp_prob <- rank(-res_k[ids_to_change])
    # }
